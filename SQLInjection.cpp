...
bool run_query(sqlite3* db, const std::string& sql, std::vector<user_record>& records)
{
    ...
    // Convert the SQL query to lowercase for case-insensitive comparison
    std::string localCopy(sql);
    std::transform(localCopy.begin(), localCopy.end(), localCopy.begin(), ::tolower);

    // Check for SQL injection patterns
    if (localCopy.find(" or ") != std::string::npos)
    {
        std::string afterOr = localCopy.substr(localCopy.find(" or ") + 4);
        if (afterOr.find("=1") != std::string::npos || afterOr.find("='") != std::string::npos)
        {
            std::cout << "SQL Injection detected and prevented: " << sql << std::endl;
            return false;
        }
    }
...
// main function with exception handling added
int main()
{
    try
    {
